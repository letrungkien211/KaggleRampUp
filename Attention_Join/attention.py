...
class Decoder:
    def __init__(self, max_len_input, max_words_target, hidden_dim):
        ...
        self.dense = Dense(max_words_target, name='DecoderDense', activation='softmax')
...
class InferenceModel():
    ...
    def save(self, prefix='./'):
        tf.keras.models.save_model(self.encoder_model, prefix+ 'inference_encoder.h5')
        tf.keras.models.save_model(self.decoder_model,prefix+ 'inference_decoder.h5')

    @staticmethod
    def load(prefix='./'):
        encoder = tf.keras.models.load_model(prefix+ 'inference_encoder.h5' )
        decoder = tf.keras.models.load_model(prefix+ 'inference_decoder.h5' )
        return InferenceModel(encoder, decoder)
...
class ModelFactory:
    ...
    def create(max_len_input, 
        ...
        initial_s_input = Input(shape=(hidden_dim_decoder,), name='DecoderHiddenState')
        initial_c_input = Input(shape=(hidden_dim_decoder,), name='DecoderCellState')

        ## Create train model

        # Encoder
        encoder_embedding_layer = Embedding(max_words_input, embedding_dim_encoder, input_length=max_len_input, name='EncoderEmbedding')
        encoder_embedding_output = encoder_embedding_layer(encoder_input)
        encoder_birnn_layer = Bidirectional(LSTM(hidden_dim_encoder, return_sequences=True), name='EncoderBiRnn')
        encoder_outputs = encoder_birnn_layer(encoder_embedding_output)

        # Decoder
        decoder_embedding_layer = Embedding(max_words_target, embedding_dim_decoder, name='DecoderEmbedding')
        ...
        # Train model
        model = Model(inputs=[encoder_input,decoder_input,initial_s_input,initial_c_input ], outputs=probs)

        ## Create inference model
        encoder_model = Model(inputs = encoder_input, outputs = encoder_outputs) # Run once per generation
        ...
        decoder_model = Model(inputs=[encoder_outputs_as_input, decoder_single_input, initial_s_input, initial_c_input], outputs=[decoder_single_output, decoder_single_h, decoder_single_c])
